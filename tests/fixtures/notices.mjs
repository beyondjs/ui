...
export class Notices {
	...
	get adapter() {
		...
		return {
			summary: async () => call('summary') ?? (this.absent ? { available: false } : { unread: this.visible.filter(item => !item.read).length, unavailable: this.missing }),
	...
	#page({ state = 'all', product = null, cursor = null, limit = 20 }) {
		...
		return { items: found.slice(start, start + limit).map(item => ({ ...item })), next: start + limit < found.length ? start + limit : null, unavailable: this.missing };
	}
