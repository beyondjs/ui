 ...
 * freely; `label`, `multiple`, `filters`, `name`, `hint`, `limit` and `labels` shape the component and
 ...
 */
export const Picker = forwardRef(function Picker({ label, source, onChange, multiple = true, selected = [], filters = [], name = null, hint = null, limit = 20, delay = 250, labels }, ref) {
	const latest = useLatest(source);
	const changed = useLatest(onChange);
	const [host, picker] = useInstance(
		() => new Search({ label, multiple, selected, filters, name, hint, limit, delay, labels, source: request => latest.current(request), onchange: items => changed.current?.(items) }),
		[label, multiple, name, hint, limit, delay, JSON.stringify(filters), labels]
	);
	useImperativeHandle(
		ref,
		() => ({
